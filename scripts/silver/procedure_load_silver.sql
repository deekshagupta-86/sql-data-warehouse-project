...
===============================================================================
	...
	*****************************************************
	...
	Purpose:
		This script creates a stored procedure that performs the ETL (Extrat, Transform, Load) process on data from the 
		Bronze layer schema. This procedure cleans and transforms the data before loading it into the Silver layer tables.

	Actions Performed:
		Drops existing Silver tables (if present).
		Re-creates Silver tables with updated schema.

	Parameters:
		None.
		This script does not accept any parameters or return values.
...
BEGIN
	...
	BEGIN TRY
		...
		-- Loading the table silver.crm_cust_info
		...
		CASE WHEN UPPER(TRIM(cst_marital_status)) = 'S' THEN 'Single'
			...
			ELSE 'n/a'
		END cst_marital_status, -- Normalize marital status values to readable format(ex- single, married)
		CASE WHEN UPPER(TRIM(cst_gndr)) = 'F' THEN 'Female'
			WHEN UPPER(TRIM(cst_gndr)) = 'M' THEN 'Male'
			ELSE 'n/a'
		END cst_gndr, -- Normalize gender values to readable format(ex- Male, Female)
		cst_create_date
		FROM(
			SELECT
			*,
		...
		PRINT '>>>>>>>>>>>Total duration of the silver layer: ' + CAST(DATEDIFF(SECOND, @batch_start_time, @batch_end_time) AS NVARCHAR) + 'seconds'; 
		PRINT '================================================';
	END TRY
	BEGIN CATCH
		PRINT '************************************************';
		PRINT 'Error occured at the time of loading'
